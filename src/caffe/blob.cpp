...
void Blob<Dtype>::Reshape(const vector<int>& shape) {
  DCHECK_LE(shape.size(), kMaxBlobAxes);
  count_ = 1;
  ...
  for (int i = 0; i < shape.size(); ++i) {
    DCHECK_GE(shape[i], 0);
    if (count_ != 0) {
      DCHECK_LE(shape[i], INT_MAX / count_) << "blob size exceeds INT_MAX";
    }
    count_ *= shape[i];
    shape_[i] = shape[i];
...
void Blob<Dtype>::Reshape(const BlobShape& shape) {
  DCHECK_LE(shape.dim_size(), kMaxBlobAxes);
...
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape) {
  if (reshape) {
    ...
    DCHECK(ShapeEquals(proto)) << proto.GetTypeName() << " shape mismatch (reshape not set)";
  }
  // copy data
  Dtype* data_vec = mutable_cpu_data();
  if (proto.double_data_size() > 0) {
    DCHECK_EQ(count_, proto.double_data_size());
    for (int i = 0; i < count_; ++i) {
      data_vec[i] = proto.double_data(i);
    }
  } else {
    DCHECK_EQ(count_, proto.data_size());
    for (int i = 0; i < count_; ++i) {
      data_vec[i] = proto.data(i);
    }
  }
  if (proto.double_diff_size() > 0) {
    DCHECK_EQ(count_, proto.double_diff_size());
    Dtype* diff_vec = mutable_cpu_diff();
    for (int i = 0; i < count_; ++i) {
      diff_vec[i] = proto.double_diff(i);
    }
  } else if (proto.diff_size() > 0) {
    DCHECK_EQ(count_, proto.diff_size());
    Dtype* diff_vec = mutable_cpu_diff();
    for (int i = 0; i < count_; ++i) {
